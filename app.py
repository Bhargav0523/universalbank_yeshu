...
# ─── LOAD MODELS (top-level so available across all tabs) ────────────────────
with st.spinner("Training classification models... This may take a moment on first load."):
    model_results, best_model_name, X_test, y_test, scaler, feature_cols = get_models()
...
with tab2:
    ...
    with col_v1:
        ...
        _scatter_df = df.sample(min(len(df), 2000), random_state=42).copy()
        _scatter_df['Loan Status'] = _scatter_df['Personal Loan'].map({0: 'Rejected', 1: 'Accepted'})
        fig_scatter = px.scatter(
            ...
            labels={'CCAvg': 'CC Avg Spend (K$)', 'Loan Status': 'Loan Status'}
        )
    ...
    findings = [
        ("💰 Income Gap", f"Acceptors earn on average <strong>${acc['Income'].mean():.0f}K</strong> vs <strong>${rej['Income'].mean():.0f}K</strong> for rejectors — a {(acc['Income'].mean()/rej['Income'].mean()-1)*100:.0f}% difference"),
        ("💳 Credit Card Spending", f"Acceptors spend <strong>${acc['CCAvg'].mean():.2f}K/month</strong> on credit cards vs <strong>${rej['CCAvg'].mean():.2f}K</strong> — nearly {acc['CCAvg'].mean()/rej['CCAvg'].mean():.1f}x more"),
        ("🏠 Mortgage", f"Acceptors hold higher mortgages: <strong>${acc['Mortgage'].mean():.0f}K</strong> vs <strong>${rej['Mortgage'].mean():.0f}K</strong>, indicating higher financial engagement"),
        ("🏦 CD Account", f"Customers with a CD account accept at <strong>{df_full[df_full['CD Account']==1]['Personal Loan'].mean()*100:.1f}%</strong> rate vs only <strong>{df_full[df_full['CD Account']==0]['Personal Loan'].mean()*100:.1f}%</strong> without one"),
        ("🎓 Education", f"Advanced degree holders accept at <strong>{df_full[df_full['Education']==3]['Personal Loan'].mean()*100:.1f}%</strong> vs Undergrads at <strong>{df_full[df_full['Education']==1]['Personal Loan'].mean()*100:.1f}%</strong>"),
    ]
